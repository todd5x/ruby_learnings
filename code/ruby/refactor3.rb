def add_bday(bday)
  bday = bday.to_s.chars.map(&:to_i) # iterate through digits of integer by turning it into an array
  bday = bday.reduce( :+ ) # reduces elements in an array to their sum
  if bday > 9
...
def message(bday)
 path = add_bday(bday) # sends user input to add_bday method to get single number
 case path # determines what to output based on value returned to and set by 'path'
  when 1
  ...
  when 9
    ...
    puts "Not sure what #{path} means!"
 end
end

puts "Please give me your birthdate in the MMDDYYYY format and I will add the digits until I get a single one which will then reveal all!"
bday = gets # gets user input
message(bday) # sends user input to message method - go there now